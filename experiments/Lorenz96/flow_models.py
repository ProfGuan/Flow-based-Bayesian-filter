...
class RealNVP(nn.Module):
    ...
    def log_probs(self, u, log_jacob):
...
#         self.Px_net = self.Build_Px_nets(m, n)
        # to device
        self.A_net = self.A_net.to(device)
        self.B_net = self.B_net.to(device)
#         self.Px_net = self.Px_net.to(device)
        self.Px = 1 * torch.ones(self.m).to(device) # diagnoal values of P_{\chi}
        self.C = self.C.to(device)
        self.D = self.D.to(device)
        self.Py = 1 * torch.ones(self.n).to(device) # diagnoal values of P_{\gamma}
...
#         Px = self.Px_net(zy_new)
#         Px_var = F.softplus(Px)
...
#         torch.save(self.Px_net, model_path + '/Px_net.py')
...
#                                                         covariance_matrix=Px_var)
        ######### init samples #########
        init_state = torch.sin(2 * np.pi * (torch.arange(1, self.m+1) / self.m))
        m_0 = init_state.to(device)
